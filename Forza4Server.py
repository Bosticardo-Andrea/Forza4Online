...
def vittoria(griglia):
    ...
    return vittoria      
def connessione(server):
    print(f'[*] ascolto su {IP}:{PORT}')
    client, address = server.accept()
    print(f'[*] Connessione avvenuta con {address[0]}:{address[1]}')
    return client,address
def main():#main
    server = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    server.bind((IP,PORT))
    server.listen(5)
    client,address = connessione(server)
    os.system("cls")
    griglia = {} #creo la griglia
    for k in range (6*7): #formatto la griglia
        griglia[k] = " "

    G1 = input("inserisci il tuo nome: ")
    client.send(G1.encode())
    os.system("cls")
    print("Attendo il nome del altro giocatore...")
    G2 = client.recv(1024).decode()
    giocatori = {G1:"X",G2:"O"}
    ...
    while True:
        while(True):
            #client1,address1,nome1 = connessione(server)
            #client1.send(nome.encode())
            print(f"Tocca a te")
            m = int(input("Inserici mossa: "))#faccio inserire la mossa del primo giocatore
            cella,griglia = controllo(m,G1,griglia,giocatori)#controllo che la mossa sia valida 
            client.send(cella.encode())
            os.system("cls")#pulisco lo schermo
        ...
        if(conta >= 41):#decretoil pareggio
            print("Pareggio")    
        print("Attendo la risposta dell'avversario...")
        rigioca = client.recv(1024).decode()
        if(rigioca == "n"):
            print("fine match!")
            client.close()
            break
